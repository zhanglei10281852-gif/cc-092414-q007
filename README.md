...
- 供应商风险画像：归集检测超标、温控异常、整改逾期与投诉四类事件，按半衰期时间衰减与因子权重计算可解释评分；评分版本记录输入事件与规则版本，同一触发只产生一个确定版本；经人工复核后发布风险等级，后续批次自动按已发布等级提高抽检比例；供应商合并或停用保留全部历史。
